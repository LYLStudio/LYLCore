namespace LStudio.Utilities
{
    ...
    public static class CompressionHelper
    {
        ...
        public static void ImprovedExtractToFile(ZipArchiveEntry file,
                                                 ...
                                                 Overwrite overwriteMethod = Overwrite.IfNewer)
        {
            if (file is null) throw new System.ArgumentNullException(nameof(file));
            ...
            //If the filename is empty the entry is for a directory, that we just created, no file to extract
        ...
        {
            if (files is null || files.Count == 0) throw new System.ArgumentNullException(nameof(files));
            ...
            switch (action)
            {
                ...
                case ArchiveAction.Error:
                    ...
                    if (archiveExists)
                    {
                        throw new IOException(string.Format(CultureInfo.InvariantCulture ,"The zip file {0} already exists.", archiveFullName));
                    }
                    break;
