    ...
    public static class ObjectHelper
    {
            ...
            foreach (var destProperty in typeof(TDest).GetProperties())
            {
                ...
                try
                {
                    ...
                    if (!(srcValue is null))
                    {
                        ...
                        {
                            value = Convert.ChangeType(srcValue, targetType, CultureInfo.InvariantCulture);
                        }
                    }
            ...
            where TDest : class, TSrc
        {
            foreach (var propertyInfo in src.GetType().GetProperties())
            {
                ...
                if (value != null)
                    propertyInfo.SetValue(dest, Convert.ChangeType(value, propertyInfo.PropertyType, CultureInfo.InvariantCulture), null);
            };
        }

        //public static void ForEachProperty<T>(T value, Action<NameValue> callback)
        //{
        //    foreach (var item in value.GetType().GetProperties())
        //    {   
        //        callback(new NameValue() { Name = item.Name, Value = item.GetValue(value) });
        //    }
        //}
        ...
        /// <param name="value">對象物件</param>
        /// <param name="callback">Callback函式</param>
        public static void ForEachProperty<T>(T value, Action<PropertyInfo, NameValue> callback)
        {
            foreach (var item in value.GetType().GetProperties())
            {
                callback(item, new NameValue() { Name = item.Name, Value = item.GetValue(value) });
            }
        }
        ...
        public static void SetValue(object inputObject, string propertyName, object propertyVal)
        {
            ...
            {
                propertyVal = Convert.ChangeType(propertyVal, targetType, CultureInfo.InvariantCulture);
            }
